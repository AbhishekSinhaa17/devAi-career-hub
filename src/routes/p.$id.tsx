...
function PublicPortfolio() {
  ...
  if (isLoading) return <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center"><Sparkles className="h-8 w-8 text-primary animate-pulse" /></div>;
  if (error || !data) return <div className="min-h-screen bg-[#0a0a0a] flex items-center justify-center text-white font-bold text-xl">Portfolio Not Found</div>;
      ...
      <div className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="relative max-w-5xl mx-auto px-6 pt-32 pb-24 flex flex-col items-center text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black tracking-tight text-white mb-6">
            Hi, I'm <span className="bg-clip-text text-transparent bg-gradient-to-r from-primary to-indigo-400">{resume.fullName}</span>
          </h1>
          ...
          <div className="mt-12 flex gap-4">
            <a href={`https://github.com/${data.github_username}`} target="_blank" rel="noreferrer" className="px-6 py-3 bg-white text-black font-bold rounded-xl flex items-center gap-2 hover:bg-slate-200 transition-colors">
        ...
        <section>
          ...
          <div className="flex flex-wrap gap-3">
            {resume.skills?.map((skill: string, i: number) => (
              <div key={i} className="px-5 py-2.5 rounded-xl bg-white/5 border border-white/10 text-sm font-semibold hover:bg-white/10 hover:border-primary/30 transition-all cursor-default">
                {skill}
              </div>
            ))}
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-6">
            {resume.projects?.map((proj: any, i: number) => (
              <div key={i} className="group p-8 rounded-3xl bg-white/[0.02] border border-white/10 hover:bg-white/[0.04] transition-all duration-300 relative overflow-hidden">
                ...
                <h3 className="text-2xl font-bold text-white mb-3 group-hover:text-primary transition-colors">{proj.name || proj.title}</h3>
