...
const getHealthScoreHistory_createServerFn_handler = createServerRpc({
  id: "7bf2392bb02ebdc447e0695910b88a0348eb4cf720f927650480f68bade850ba",
  name: "getHealthScoreHistory",
  filename: "src/lib/health.functions.ts"
}, (opts) => getHealthScoreHistory.__executeServer(opts));
const getHealthScoreHistory = createServerFn({
  method: "GET"
}).middleware([requireAuth]).validator((d) => objectType({}).parse(d ?? {})).handler(getHealthScoreHistory_createServerFn_handler, async ({
  context
}) => {
  const {
    data
  } = await serverApiClient.get("/health-score/history", {
    headers: {
...
const generateHealthScore_createServerFn_handler = createServerRpc({
  id: "50a4b35d1bf5a89b05030680c829673b6515c79336e4fb2d0a699e2aa448f775",
  name: "generateHealthScore",
  filename: "src/lib/health.functions.ts"
}, (opts) => generateHealthScore.__executeServer(opts));
const generateHealthScore = createServerFn({
  method: "POST"
}).middleware([requireAuth]).validator((d) => objectType({}).parse(d ?? {})).handler(generateHealthScore_createServerFn_handler, async ({
  context
}) => {
  const {
    data
  } = await serverApiClient.post("/health-score/generate", {}, {
    headers: {
      Authorization: `Bearer ${context.token}`
    }
  });
...
export {
  generateHealthScore_createServerFn_handler,
  getHealthScoreHistory_createServerFn_handler
};
