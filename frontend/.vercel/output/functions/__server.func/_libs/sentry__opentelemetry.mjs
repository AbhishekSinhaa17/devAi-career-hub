...
import { ac as setAsyncContextStrategy, ad as spanToJSON, ae as SEMANTIC_ATTRIBUTE_SENTRY_SOURCE, a1 as hasSpansEnabled, af as spanToTraceContext, ag as getRootSpan, ah as getDynamicSamplingContextFromSpan, ai as addChildSpanToSpan, aj as getDefaultIsolationScope, ak as getDefaultCurrentScope, al as setCapturedScopesOnSpan, am as logSpanStart, b as getClient, an as logSpanEnd, ao as hasSpanStreamingEnabled, ap as shouldIgnoreSpan, c as SEMANTIC_ATTRIBUTE_SENTRY_OP, aq as baggageHeaderToDynamicSamplingContext, ar as parseSampleRate, as as safeMathRandom, at as sampleSpan, d as debug, au as SEMANTIC_ATTRIBUTE_SENTRY_SAMPLE_RATE, k as SDK_VERSION, L as LRUMap, r as shouldPropagateTraceForUrl, av as parseBaggageHeader, aw as SENTRY_BAGGAGE_KEY_PREFIX, ax as generateSentryTraceHeader, ay as generateTraceparentHeader, az as getTraceContextFromScope, aA as getDynamicSamplingContextFromScope, aB as safeDateNow, aC as debounce, aD as timedEventsToMeasurements, aE as captureEvent, a2 as getCurrentScope, i as getIsolationScope, a6 as propagationContextFromHeaders, aF as shouldContinueTrace, aG as derefWeakRef, aH as getCapturedScopesOnSpan, aI as convertSpanLinksForEnvelope, aJ as getStatusMessage, aK as spanTimeInputToSeconds, S as SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, aL as SEMANTIC_ATTRIBUTE_SENTRY_CUSTOM_SPAN_NAME, a as addNonEnumerableProperty, aM as makeWeakRef, aN as SPAN_STATUS_OK, f as SPAN_STATUS_ERROR, w as parseUrl, x as getSanitizedUrlString, s as stripUrlQueryAndFragment, aO as dynamicSamplingContextToSentryBaggageHeader, e as getSpanStatusFromHttpCode, aP as generateTraceId, aQ as generateSpanId, aR as handleCallbackErrors, aS as warn } from "./sentry__core.mjs";
import { S as SamplingDecision } from "./opentelemetry__sdk-trace-base.mjs";
import { S as SDK_INFO, W as W3CBaggagePropagator, i as isTracingSuppressed, a as suppressTracing$1 } from "./opentelemetry__core.mjs";
...
function getRequestSpanData(span) {
  ...
  const maybeUrlAttribute = span.attributes[srcExports$1.ATTR_URL_FULL] || span.attributes[srcExports$1.SEMATTRS_HTTP_URL];
  const data = {
    url: maybeUrlAttribute,
    // eslint-disable-next-line deprecation/deprecation
    "http.method": span.attributes[srcExports$1.ATTR_HTTP_REQUEST_METHOD] || span.attributes[srcExports$1.SEMATTRS_HTTP_METHOD]
  };
...
function getSpanKind(span) {
  ...
  return srcExports.SpanKind.INTERNAL;
}
...
const SENTRY_SCOPES_CONTEXT_KEY = srcExports.createContextKey("sentry_scopes");
const SENTRY_FORK_ISOLATION_SCOPE_CONTEXT_KEY = srcExports.createContextKey("sentry_fork_isolation_scope");
const SENTRY_FORK_SET_SCOPE_CONTEXT_KEY = srcExports.createContextKey("sentry_fork_set_scope");
const SENTRY_FORK_SET_ISOLATION_SCOPE_CONTEXT_KEY = srcExports.createContextKey("sentry_fork_set_isolation_scope");
...
function getSamplingDecision(spanContext) {
  ...
  if (traceFlags === srcExports.TraceFlags.SAMPLED) {
    return true;
  }
...
function inferSpanData(spanName, attributes, kind) {
  const httpMethod = attributes[srcExports$1.ATTR_HTTP_REQUEST_METHOD] || attributes[srcExports$1.SEMATTRS_HTTP_METHOD];
  if (httpMethod) {
    return descriptionForHttpMethod({ attributes, name: spanName, kind }, httpMethod);
  }
  const dbSystem = attributes[srcExports$1.ATTR_DB_SYSTEM_NAME] || attributes[srcExports$1.SEMATTRS_DB_SYSTEM];
  ...
  const rpcService = attributes[srcExports$1.SEMATTRS_RPC_SERVICE];
  if (rpcService) {
    return {
  ...
  const messagingSystem = attributes[srcExports$1.SEMATTRS_MESSAGING_SYSTEM];
  if (messagingSystem) {
    return {
  ...
  const faasTrigger = attributes[srcExports$1.SEMATTRS_FAAS_TRIGGER];
  if (faasTrigger) {
    return {
      ...getUserUpdatedNameAndSource(spanName, attributes, customSourceOrRoute),
...
function descriptionForDbSystem({ attributes, name }) {
  ...
  const statement = attributes[srcExports$1.SEMATTRS_DB_STATEMENT];
...
function descriptionForHttpMethod({ name, kind, attributes }, httpMethod) {
  const opParts = ["http"];
  switch (kind) {
    case srcExports.SpanKind.CLIENT:
      opParts.push("client");
      break;
    case srcExports.SpanKind.SERVER:
      opParts.push("server");
      break;
  }
  ...
  const isClientOrServerKind = kind === srcExports.SpanKind.CLIENT || kind === srcExports.SpanKind.SERVER;
...
function getSanitizedUrl(attributes, kind) {
  const httpTarget = attributes[srcExports$1.SEMATTRS_HTTP_TARGET];
  const httpUrl = attributes[srcExports$1.SEMATTRS_HTTP_URL] || attributes[srcExports$1.ATTR_URL_FULL];
  const httpRoute = attributes[srcExports$1.ATTR_HTTP_ROUTE];
  ...
  if (kind === srcExports.SpanKind.SERVER && typeof httpTarget === "string") {
...
function getActiveSpan() {
  return srcExports.trace.getActiveSpan();
}
class TraceState {
  constructor() {
...
class SentryPropagator extends W3CBaggagePropagator {
  ...
  inject(context2, carrier, setter) {
    ...
    const activeSpan = srcExports.trace.getSpan(context2);
    ...
    let baggage = srcExports.propagation.getBaggage(context2) || srcExports.propagation.createBaggage({});
    ...
    if (!existingSentryTraceHeader && traceId && traceId !== srcExports.INVALID_TRACEID) {
      setter.set(carrier, SENTRY_TRACE_HEADER, generateSentryTraceHeader(traceId, spanId, sampled));
      if (propagateTraceparent) {
        setter.set(carrier, "traceparent", generateTraceparentHeader(traceId, spanId, sampled));
      }
    }
    super.inject(srcExports.propagation.setBaggage(context2, baggage), carrier, setter);
  }
  /**
   * @inheritDoc
   */
...
function getInjectionData(context2, options = {}) {
  const span = srcExports.trace.getSpan(context2);
...
function getContextWithRemoteActiveSpan(ctx, { sentryTrace, baggage }) {
  ...
  return srcExports.trace.setSpanContext(ctx, spanContext);
}
function continueTraceAsRemoteSpan(ctx, options, callback) {
  const ctxWithSpanContext = ensureScopesOnContext(getContextWithRemoteActiveSpan(ctx, options));
  return srcExports.context.with(ctxWithSpanContext, callback);
}
...
function getCurrentURL(span) {
  ...
  const urlAttribute = spanData[srcExports$1.SEMATTRS_HTTP_URL] || spanData[srcExports$1.ATTR_URL_FULL];
...
function generateRemoteSpanContext({
  ...
  const spanContext = {
    ...
    spanId,
    isRemote: true,
    traceFlags: sampled ? srcExports.TraceFlags.SAMPLED : srcExports.TraceFlags.NONE,
    traceState
  };
  return spanContext;
}
function _startSpan(options, callback, autoEnd) {
  ...
  return wrapper(() => {
    ...
    const missingRequiredParent = options.onlyIfParent && !srcExports.trace.getSpan(activeCtx);
    ...
    if (!hasSpansEnabled()) {
      ...
      return srcExports.context.with(suppressedCtx, () => {
        return tracer.startActiveSpan(name, spanOptions, suppressedCtx, (span) => {
          patchSpanEnd(span);
          return srcExports.context.with(activeCtx, () => {
...
function withActiveSpan(span, callback) {
  const newContextWithActiveSpan = span ? srcExports.trace.setSpan(srcExports.context.active(), span) : srcExports.trace.deleteSpan(srcExports.context.active());
  return srcExports.context.with(newContextWithActiveSpan, () => callback(getCurrentScope()));
}
function getTracer() {
  const client = getClient();
  return client?.tracer || srcExports.trace.getTracer("@sentry/opentelemetry", SDK_VERSION);
}
...
function getContext(scope, forceTransaction) {
  ...
  const parentSpan = srcExports.trace.getSpan(ctx);
  if (!parentSpan) {
    return ctx;
  }
  if (!forceTransaction) {
  ...
  const ctxWithoutSpan = srcExports.trace.deleteSpan(ctx);
  ...
  const ctxWithSpanContext = srcExports.trace.setSpanContext(ctxWithoutSpan, spanOptions);
...
function getContextForScope(scope) {
  ...
  return srcExports.context.active();
}
function continueTrace(options, callback) {
  return continueTraceAsRemoteSpan(srcExports.context.active(), options, callback);
}
function startNewTrace(callback) {
  ...
  const spanContext = {
    ...
    spanId,
    isRemote: true,
    traceFlags: srcExports.TraceFlags.NONE
  };
  const ctxWithTrace = srcExports.trace.setSpanContext(srcExports.context.active(), spanContext);
  return srcExports.context.with(ctxWithTrace, () => {
...
function getTraceContextForScope(client, scope) {
  const ctx = getContextFromScope(scope);
  const span = ctx && srcExports.trace.getSpan(ctx);
...
function suppressTracing(callback) {
  const ctx = suppressTracing$1(srcExports.context.active());
  return srcExports.context.with(ctx, callback);
}
...
function getTraceData({
  ...
  let ctx = (scope && getContextFromScope(scope)) ?? srcExports.context.active();
  if (span) {
    const { scope: scope2 } = getCapturedScopesOnSpan(span);
    ctx = scope2 && getContextFromScope(scope2) || srcExports.trace.setSpan(srcExports.context.active(), span);
  }
...
function setOpenTelemetryContextAsyncContextStrategy() {
  ...
  function withScope(callback) {
    const ctx = srcExports.context.active();
    return srcExports.context.with(ctx, () => {
      return callback(getCurrentScope2());
    });
  }
  function withSetScope(scope, callback) {
    const ctx = getContextFromScope(scope) || srcExports.context.active();
    return srcExports.context.with(ctx.setValue(SENTRY_FORK_SET_SCOPE_CONTEXT_KEY, scope), () => {
      return callback(scope);
    });
  }
  function withIsolationScope(callback) {
    const ctx = srcExports.context.active();
    return srcExports.context.with(ctx.setValue(SENTRY_FORK_ISOLATION_SCOPE_CONTEXT_KEY, true), () => {
      return callback(getIsolationScope2());
    });
  }
  function withSetIsolationScope(isolationScope, callback) {
    const ctx = srcExports.context.active();
    return srcExports.context.with(ctx.setValue(SENTRY_FORK_SET_ISOLATION_SCOPE_CONTEXT_KEY, isolationScope), () => {
      return callback(getIsolationScope2());
    });
  }
...
function buildContextWithSentryScopes(context, activeContext) {
  const span = srcExports.trace.getSpan(context);
  let effectiveContext;
  if (span?.spanContext().traceState?.get(SENTRY_TRACE_STATE_CHILD_IGNORED) === "1") {
    const contextWithoutSpan = srcExports.trace.deleteSpan(context);
    const parentSpan = srcExports.trace.getSpan(activeContext);
    effectiveContext = parentSpan ? srcExports.trace.setSpan(contextWithoutSpan, parentSpan) : contextWithoutSpan;
  } else {
...
function mapStatus(span) {
  ...
  if (status) {
    if (status.code === srcExports.SpanStatusCode.OK) {
      return { code: SPAN_STATUS_OK };
    } else if (status.code === srcExports.SpanStatusCode.ERROR) {
...
function inferStatusFromAttributes(attributes) {
  const httpCodeAttribute = attributes[srcExports$1.ATTR_HTTP_RESPONSE_STATUS_CODE] || attributes[srcExports$1.SEMATTRS_HTTP_STATUS_CODE];
  const grpcCodeAttribute = attributes[srcExports$1.SEMATTRS_RPC_GRPC_STATUS_CODE];
...
function createTransactionForOtelSpan(span) {
  ...
  const statusCode = attributes[srcExports$1.ATTR_HTTP_RESPONSE_STATUS_CODE];
...
function getData(span) {
  const attributes = span.attributes;
  const data = {};
  if (span.kind !== srcExports.SpanKind.INTERNAL) {
    data["otel.kind"] = srcExports.SpanKind[span.kind];
  }
  const maybeHttpStatusCodeAttribute = attributes[srcExports$1.SEMATTRS_HTTP_STATUS_CODE];
  if (maybeHttpStatusCodeAttribute) {
    data[srcExports$1.ATTR_HTTP_RESPONSE_STATUS_CODE] = maybeHttpStatusCodeAttribute;
  }
...
class SentrySpanProcessor {
  ...
  onStart(span, parentContext) {
    const parentSpan = srcExports.trace.getSpan(parentContext);
    ...
    if (parentContext === srcExports.ROOT_CONTEXT) {
      scopes = {
        scope: getDefaultCurrentScope(),
        isolationScope: getDefaultIsolationScope()
      };
...
class SentrySampler {
  ...
  shouldSample(context, traceId, spanName, spanKind, spanAttributes, _links) {
    ...
    const maybeSpanHttpMethod = spanAttributes[srcExports$1.SEMATTRS_HTTP_METHOD] || spanAttributes[srcExports$1.ATTR_HTTP_REQUEST_METHOD];
    if (spanKind === srcExports.SpanKind.CLIENT && maybeSpanHttpMethod && (!parentSpan || parentContext?.isRemote)) {
...
function getParentSampled(parentSpan, traceId, spanName) {
  const parentContext = parentSpan.spanContext();
  if (srcExports.isSpanContextValid(parentContext) && parentContext.traceId === traceId) {
...
function getBaseTraceState(context, spanAttributes) {
  ...
  const url = spanAttributes[srcExports$1.SEMATTRS_HTTP_URL] || spanAttributes[srcExports$1.ATTR_URL_FULL];
...
function getValidSpan(context) {
  const span = srcExports.trace.getSpan(context);
  return span && srcExports.isSpanContextValid(span.spanContext()) ? span : void 0;
}
...
function getSentryResource(serviceNameFallback) {
  ...
  return new SentryResource({
    ...
    // eslint-disable-next-line deprecation/deprecation
    [srcExports$1.SEMRESATTRS_SERVICE_NAMESPACE]: "sentry",
    [srcExports$1.ATTR_SERVICE_NAME]: serviceNameFallback,
    ...
    ...otelServiceName ? { [srcExports$1.ATTR_SERVICE_NAME]: otelServiceName } : {},
    // Highest priority: Sentry SDK telemetry attrs (cannot be overridden by env vars)
    [srcExports$1.ATTR_SERVICE_VERSION]: SDK_VERSION,
...
class SentryAsyncLocalStorageContextManager {
  ...
  active() {
    return this._asyncLocalStorage.getStore() ?? srcExports.ROOT_CONTEXT;
  }
...
function tracingChannel(channelNameOrInstance, transformStart) {
  ...
  let lookup;
  try {
    const contextManager = srcExports.context._getContextManager();
  ...
  channel.start.bindStore(otelStorage, (data) => {
    ...
    return srcExports.trace.setSpan(srcExports.context.active(), span);
  });
  return channel;
}
export {
  SentryAsyncLocalStorageContextManager as S,
  setupEventContextTrace as a,
  SEMANTIC_ATTRIBUTE_SENTRY_GRAPHQL_OPERATION as b,
  SentrySpanProcessor as c,
  SentrySampler as d,
  enhanceDscWithOpenTelemetryRootSpanName as e,
  getSentryResource as f,
  getTraceContextForScope as g,
  SentryPropagator as h,
  openTelemetrySetupCheck as o,
  setOpenTelemetryContextAsyncContextStrategy as s,
  tracingChannel as t
};
