...
import { l as logs } from "./opentelemetry__api-logs.mjs";
...
function registerInstrumentations(options) {
  const tracerProvider = options.tracerProvider || srcExports.trace.getTracerProvider();
  const meterProvider = options.meterProvider || srcExports.metrics.getMeterProvider();
...
function satisfies(version, range, options) {
  if (!_validateVersion(version)) {
    srcExports.diag.error(`Invalid version: ${version}`);
    return false;
  }
  if (!range) {
    return true;
...
function _parseVersion(versionString) {
  ...
  if (!match) {
    srcExports.diag.error(`Invalid version: ${versionString}`);
    return void 0;
  }
...
function _parseRange(rangeString) {
  ...
  if (!match) {
    srcExports.diag.error(`Invalid range: ${rangeString}`);
    return {
      invalid: true
    };
  }
...
class InstrumentationAbstract {
  ...
  constructor(instrumentationName, instrumentationVersion, config) {
    ...
    this._diag = srcExports.diag.createComponentLogger({
      namespace: instrumentationName
    });
    this._tracer = srcExports.trace.getTracer(instrumentationName, instrumentationVersion);
    this._meter = srcExports.metrics.getMeter(instrumentationName, instrumentationVersion);
...
class InstrumentationBase extends InstrumentationAbstract {
  ...
  _massWrap = (moduleExportsArray, names, wrapper) => {
    if (!moduleExportsArray) {
      srcExports.diag.error("must provide one or more modules to patch");
      return;
    } else if (!Array.isArray(moduleExportsArray)) {
      moduleExportsArray = [moduleExportsArray];
    }
    if (!(names && Array.isArray(names))) {
      srcExports.diag.error("must provide one or more functions to wrap on modules");
      return;
    }
  ...
  _extractPackageVersion(baseDir) {
    try {
      ...
      srcExports.diag.warn("Failed extracting version", baseDir);
    }
    return void 0;
  }
...
export {
  ...
  SemconvStability as S,
  InstrumentationNodeModuleDefinition as a,
  semconvStabilityFromStr as b,
  isWrapped as i,
  registerInstrumentations as r,
