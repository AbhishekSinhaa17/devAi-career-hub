import { S as SentryAsyncLocalStorageContextManager, g as getTraceContextForScope, o as openTelemetrySetupCheck, s as setOpenTelemetryContextAsyncContextStrategy, e as enhanceDscWithOpenTelemetryRootSpanName, a as setupEventContextTrace } from "./sentry__opentelemetry.mjs";
import { s as srcExports } from "./@opentelemetry/api.mjs";
import { d as debug, g as getHttpServerSubscriptions, H as HTTP_ON_SERVER_REQUEST, a as addNonEnumerableProperty, b as getClient, p as parseStringToURLObject, s as stripUrlQueryAndFragment, h as httpHeadersToSpanAttributes, S as SEMANTIC_ATTRIBUTE_SENTRY_ORIGIN, c as SEMANTIC_ATTRIBUTE_SENTRY_OP, e as getSpanStatusFromHttpCode, f as SPAN_STATUS_ERROR, i as getIsolationScope, j as parseSemver, k as SDK_VERSION, l as HTTP_ON_CLIENT_REQUEST, m as patchHttpModuleClient, n as getHttpClientSubscriptions, o as getRequestOptions, q as defineIntegration, r as shouldPropagateTraceForUrl, t as getTraceData, u as mergeBaggageHeaders, v as addBreadcrumb, w as parseUrl, x as getSanitizedUrlString, y as getBreadcrumbLogLevelFromHttpStatusCode, L as LRUMap, z as captureException, A as safeSetSpanJSONAttributes, B as snipLine, C as consoleSandbox, G as GLOBAL_OBJ, D as withActiveSpan, E as isMatchingPattern, F as startSession, I as endSession, J as serializeEnvelope, K as suppressTracing, M as maybeInstrument, N as consoleIntegration$1, O as CONSOLE_LEVELS, P as originalConsoleMethods, Q as markFunctionWrapped, R as fill, T as triggerHandlers, U as createTransport, V as envToBool, W as dirname$1, X as createStackParser, Y as nodeStackLineParser, Z as ServerRuntimeClient, _ as applySdkMetadata, $ as _INTERNAL_flushLogsBuffer, a0 as _INTERNAL_clearAiProviderSkips, a1 as hasSpansEnabled, a2 as getCurrentScope, a3 as stackParserFromStackParserOptions, a4 as getIntegrationsToSetup, a5 as spanStreamingIntegration, a6 as propagationContextFromHeaders, a7 as inboundFiltersIntegration, a8 as functionToStringIntegration, a9 as linkedErrorsIntegration, aa as requestDataIntegration, ab as conversationIdIntegration } from "./sentry__core.mjs";
...
import { r as registerInstrumentations, I as InstrumentationBase, a as InstrumentationNodeModuleDefinition } from "./opentelemetry__instrumentation.mjs";
import { errorMonitor } from "node:events";
import { s as srcExports$1 } from "./@opentelemetry/semantic-conventions+[...].mjs";
import { R as RPCType, s as setRPCMetadata, i as isTracingSuppressed, g as getRPCMetadata } from "./opentelemetry__core.mjs";
...
const HTTP_SERVER_INSTRUMENTED_KEY = srcExports.createContextKey("sentry_http_server_instrumented");
...
const _httpServerIntegration = ((options = {}) => {
  const _options = {
    ...
    wrapServerEmitRequest(request, response, normalizedRequest, next) {
      ...
      if (srcExports.context.active().getValue(HTTP_SERVER_INSTRUMENTED_KEY)) {
        return next();
      }
      const ctx = srcExports.propagation.extract(srcExports.context.active(), normalizedRequest.headers).setValue(HTTP_SERVER_INSTRUMENTED_KEY, true);
      srcExports.context.with(ctx, () => {
      ...
      client.on("httpServerRequest", (_request, _response, normalizedRequest) => {
        ...
        const startSpan = (next) => {
          ...
          const span = tracer.startSpan(bestEffortTransactionName, {
            kind: srcExports.SpanKind.SERVER,
            attributes: {
          ...
          return srcExports.context.with(setRPCMetadata(srcExports.trace.setSpan(srcExports.context.active(), span), rpcMetadata), () => {
            srcExports.context.bind(srcExports.context.active(), request);
            srcExports.context.bind(srcExports.context.active(), response);
            let isEnded = false;
...
function shouldIgnoreSpansForIncomingRequest(request, {
  ...
  if (isTracingSuppressed(srcExports.context.active())) {
    return true;
  }
...
function getIncomingRequestAttributesOnResponse(request, response) {
  ...
  const newAttributes = {
    [srcExports$1.ATTR_HTTP_RESPONSE_STATUS_CODE]: statusCode,
    // eslint-disable-next-line deprecation/deprecation
    [srcExports$1.SEMATTRS_HTTP_STATUS_CODE]: statusCode,
    "http.status_text": statusMessage?.toUpperCase()
  };
  const rpcMetadata = getRPCMetadata(srcExports.context.active());
  if (socket) {
    const { localAddress, localPort, remoteAddress, remotePort } = socket;
    newAttributes[srcExports$1.SEMATTRS_NET_HOST_IP] = localAddress;
    newAttributes[srcExports$1.SEMATTRS_NET_HOST_PORT] = localPort;
    newAttributes[srcExports$1.SEMATTRS_NET_PEER_IP] = remoteAddress;
    newAttributes["net.peer.port"] = remotePort;
  }
  newAttributes[srcExports$1.SEMATTRS_HTTP_STATUS_CODE] = statusCode;
  newAttributes["http.status_text"] = (statusMessage || "").toUpperCase();
  if (rpcMetadata?.type === RPCType.HTTP && rpcMetadata.route !== void 0) {
    const routeName = rpcMetadata.route;
    newAttributes[srcExports$1.ATTR_HTTP_ROUTE] = routeName;
  }
  ...
  init() {
    ...
    const patchOptions = {
      ...
      ignoreOutgoingRequests(url, request) {
        return isTracingSuppressed(srcExports.context.active()) || !!options.ignoreOutgoingRequests?.(url, getRequestOptions(request));
      },
      outgoingRequestHook(span, request) {
        ...
        const newOnce = new Proxy(originalOnce, {
          apply(target, thisArg, args) {
            ...
            const parentContext = srcExports.context.active();
            const requestContext = srcExports.trace.setSpan(parentContext, span);
            return srcExports.context.with(requestContext, () => {
              return target.apply(thisArg, args);
            });
          }
        });
...
function setupOpenTelemetryLogger() {
  srcExports.diag.disable();
  srcExports.diag.setLogger(
    {
      error: debug.error,
      warn: debug.warn,
      info: debug.log,
    ...
    srcExports.DiagLogLevel.DEBUG
  );
}
const INTEGRATION_NAME$9 = "ChildProcess";
const childProcessIntegration = defineIntegration((options = {}) => {
  return {
    name: INTEGRATION_NAME$9,
    setup() {
      dc.channel("child_process").subscribe((event) => {
        if (event && typeof event === "object" && "process" in event) {
          captureChildProcessEvents(event.process, options);
        }
      });
      dc.channel("worker_threads").subscribe((event) => {
...
const INTEGRATION_NAME$8 = "Context";
...
const INTEGRATION_NAME$7 = "ContextLines";
...
const INTEGRATION_NAME$6 = "LocalVariables";
...
let moduleCache;
const INTEGRATION_NAME$5 = "Modules";
...
const INTEGRATION_NAME$4 = "OnUncaughtException";
...
const INTEGRATION_NAME$2 = "ProcessSession";
...
const INTEGRATION_NAME$1 = "Spotlight";
...
class NodeClient extends ServerRuntimeClient {
  ...
  get tracer() {
    ...
    const tracer = srcExports.trace.getTracer(name, version);
    this._tracer = tracer;
    return tracer;
  }
  /** @inheritDoc */
...
function _init(_options = {}, getDefaultIntegrationsImpl) {
  ...
  if (options.spotlight && !options.integrations.some(({ name }) => name === INTEGRATION_NAME$1)) {
...
export {
  ...
  SentryNodeFetchInstrumentation as b,
  addOriginToSpan as c,
  SentryContextManager as d,
  init as e,
