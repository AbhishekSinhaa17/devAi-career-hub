export function generatePortfolioFiles(resumeData: any) {
  ...
  const packageJson = {
    name: `portfolio-${fullName?.toLowerCase().replace(/[^a-z0-9]/g, "-") || "app"}`,
    version: "1.0.0",
    private: true,
    scripts: {
      dev: "vite",
...
<html lang="en">
  <head>
    ...
    <title>${fullName || 'Developer'} - ${title || 'Portfolio'}</title>
  </head>
  <body>
    <div id="root"></div>
      ...
      <div className="relative overflow-hidden border-b border-white/5">
        ...
        <div className="relative max-w-5xl mx-auto px-6 pt-32 pb-24 flex flex-col items-center text-center">
          ...
          <h1 className="text-5xl md:text-7xl font-black tracking-tight text-white mb-6">
            Hi, I'm <span className="bg-clip-text text-transparent bg-gradient-to-r from-indigo-400 to-purple-400">{resume.fullName || 'Developer'}</span>
          </h1>
